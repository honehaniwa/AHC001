...
fn vis(input: &Input, out: &Vec<Rect>, show_id: bool, name: String) {
	...
	if name != ""{
		svg::save(format!("./out/out{}.svg", name), &doc).unwrap();
	}
	else{
		svg::save(format!("out{}.svg", name), &doc).unwrap();
	}
}

fn main() {
	if std::env::args().len() < 3 || std::env::args().len() > 5{
		eprintln!("Usage: {} <input> <output> (index)", std::env::args().nth(0).unwrap());
		return;
	}
	...
	if std::env::args().len() == 4{
		vis(&input, &output, true, std::env::args().nth(3).unwrap().to_string());
	}
	else {
		vis(&input, &output, true, "".to_string());
	}
}
